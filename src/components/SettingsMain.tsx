...
import { tasksByDate } from "@/constants/constants";
...
export default function SettingsMain() {
  ...
  const [selectedView, setSelectedView] = useState<
    "List" | "Board" | "Timeline"
  >("Timeline");
  const [tasks] = useState<Record<string, TaskType[]>>(tasksByDate);
      ...
      <div className="flex">
        ...
        <div className="flex-1 bg-white mt-2 mx-4 p-2 overflow-auto">
          ...
          {Array.from({ length: 9 }, (_, i) => i + 9).map((hour, index) => {
            const tasksForDate = tasks[currentDate.toDateString()] || [];
            const taskAtThisHour = tasksForDate.find((t) => t.time === hour);
